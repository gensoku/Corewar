...
/*   Updated: 2017/08/10 06:21:04 by cchaumar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <corewar.h>

static int		tok_func_path(char **s)
{
	if (*s && !**s)
		return (1);
...
static int		tok_func_verbose(char **s)
{
	int		n;
	char	*p;
...
static int		tok_func_none(char **s)
{
	(void)s;
	return (0);
}

static int		tok_func_number(char **s)
{
	char	*p;
	int		n;
...
static int		tok_func_interrupt(char **s)
{
	if (ft_strncmp(*s, "--", 2))
		return (1);
	if ((*s)[2] && (*s)[2] != ' ')
...
t_token	g_crwr_opts[] = {
	...
	{"INTERRUPT", tok_func_interrupt, 0, TOKEN_TYPE_FNC},
